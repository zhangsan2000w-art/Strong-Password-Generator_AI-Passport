...
| Verified 520mAh CW2017 profile | A fixed built-in-profile SOC can stay plausible but fail to follow the fitted cell; the supplied 80-byte profile is only installed when its flag or contents differ | Trust an unmatched profile; reset the gauge every boot; voltage only | Firmware verifies all profile bytes, preserves the alert threshold, and waits at most five seconds for SOC; MoonBit tests cover display fallback | Accuracy and movement still require a full device charge/discharge check |
