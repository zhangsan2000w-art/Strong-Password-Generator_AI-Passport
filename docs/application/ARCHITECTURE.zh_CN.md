...
FreeRTOS 输入任务 + LVGL/硬件适配层（C）
      |
MoonBit 产品核心：生成、状态、视图、HID 映射、电量、强度、声音
      |
C FFI：原始硬件输入与输出
      |
...
| `ble_keyboard.mbt` | 发送可用性、传输状态展示策略和可显示 ASCII 到 USB HID 的映射 |
...
`password_ble_keyboard.c` 只负责 NimBLE/HID 传输。GAP 回调只发布传输状态，NimBLE 使用无需配对码的 Just Works 完成绑定和加密；明确的 MoonBit “发送”动作把当前显示缓冲复制到深度为 1 的 FreeRTOS 队列，独立 worker 通过 MoonBit HID 映射逐字符发送按下／释放 report，并在结束后清零临时缓冲。LVGL timer 只读取状态，不发送或记录密码。
...
| 校验并使用 CW2017 520mAh 电芯 profile | 内置 profile 的固定 SOC 可能数值合理却不跟随实际电芯；仅在更新标志或 80 字节内容不匹配时安装配套 profile | 信任不匹配的 profile；每次启动都复位；只显示电压 | 固件逐字节校验、保留告警阈值并限时等待 SOC；MoonBit 测试覆盖显示回退 | 准确度和变化仍需真机完整充放电验证 |
| MoonBit 合成、C 异步播放成功音 | 让产品反馈策略属于 MoonBit，同时无需音频素材和额外 Flash | C 合成、同步播放或 WAV 文件 | MoonBit 波形/包络测试及固件编译 | 音量和听感需真机确认 |
| 加密 Just Works BLE HID + 明确“发送” | 取消配对码输入，同时保留绑定加密和用户确认发送 | 配对码、剪贴板同步、BLE UART、二维码或自动输入 | MoonBit 映射／状态测试、ESP-IDF 构建，再做跨主机真机验收 | Just Works 不提供中间人认证；主机焦点、输入法／布局、删除 bond 和端到端输入仍需真机确认 |
