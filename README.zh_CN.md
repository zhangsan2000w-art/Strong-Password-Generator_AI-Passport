...
- **输入**：只使用 `UP`、`DOWN`、`OK`。`OK` 用于进入或确认编辑、切换布尔值及生成；编辑数值时长按 `UP` / `DOWN` 可连续增减；长按 `OK` 可取消当前编辑，主界面未处于编辑状态时则进入设置页。
...
- 主界面未处于编辑状态时，长按 `OK` 进入设置页。
- 使用 `UP` / `DOWN` 选择 **Theme** 或 **Sound**，按 `OK` 修改当前选项，长按 `OK` 返回主界面。
- **Theme** 可在赛博朋克深色界面与蓝天、白云、草地界面之间切换。
- **Sound** 控制成功音效，不影响密码生成。
- 主题和声音偏好从 ESP-IDF NVS 载入，并通过独立任务异步保存；生成的密码和密码历史始终不会持久化。
- NVS 不可用时，修改仍在当前会话生效，固件会记录告警；应用不会自动擦除整个 NVS 分区。

## MoonBit 主体实现

当前仓库有 1,610 行生产 `.mbt` 与 702 行 MoonBit 测试，共 2,312 行。排除测试、空行和注释后，生产 MoonBit 有效代码为 1,361 行。`tools/check_repo.py` 会独立检查生产实现不少于 1,000 有效行，测试代码不能用于凑这个门槛。
...
C 只保留 ESP-IDF/BSP 初始化、LVGL 控件绘制、I2C 原始读数、FreeRTOS 调度、NVS 持久化适配、音频写入、安全随机源及 Flash 词库访问。
...
当前代码定义了 44 项 MoonBit 测试。MoonBit 严格检查、1,361 行生产有效代码门禁、仓库检查、11 项 Python 固件布局测试、ESP-IDF 5.5.3 固件构建及合并镜像校验已在本地通过。本机检测到的旧版 Windows C 编译器找不到 `stdint.h`，因此原生 MoonBit 测试可执行文件未能构建；这是环境失败，不能记为测试通过。双主题设置页、重启后的偏好保留、声音开关、字体、按键、电池行为和 RNG 适配器仍需真机验证。
