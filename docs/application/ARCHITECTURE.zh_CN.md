...
`password_ble_keyboard.c` 只负责 NimBLE/HID 传输。GAP 回调仅发布传输状态和配对码；明确的 MoonBit “发送”动作把当前显示缓冲复制到深度为 1 的 FreeRTOS 队列，独立 worker 通过 MoonBit HID 映射逐字符发送按下／释放 report，并在结束后清零临时缓冲。LVGL timer 只读取状态，不发送或记录密码。
...
| 认证配对 BLE HID + 明确“发送” | 在不存储密码、不自动外发的前提下提供最强演示路径 | 剪贴板同步、BLE UART、二维码或自动输入 | MoonBit 映射／状态测试、ESP-IDF 构建，再做跨主机真机验收 | 主机焦点、输入法／布局、删除 bond 和端到端输入仍需真机确认 |
